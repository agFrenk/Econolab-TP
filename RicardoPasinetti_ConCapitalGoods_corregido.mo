model RicardoPasinetti_ConCapitalGoods_corregido
  ...
  parameter Real a3 = 0.4 "Elasticidad laboral sector 3";
  parameter Real delta = 0.4 "Tasa de depreciación del capital";
  parameter Real alpha3 = 0.3;
  ...
  Real N2(start = 6) "Empleo sector 2 (oro)";
  Real N3 "Empleo sector 3 (bienes de capital)";
  Real Nres; 
  Real coefPoblacionN3 = 0.2;
  // ===== Outputs =====
...
// ===== Sector 3: bienes de capital =====
  Y3 = A3 * (M3^alpha3) * (N3^a3);
  Y3_p = Y3 * p3;
  MPL3 = A3* (M3^alpha3) * a3 * N3^(a3 - 1);
  p3 = 1/MPL3;
  der(M1) = I1 - delta*M1;
  der(M3) = I3 - delta*M3;
  M = M1 + M3;
  N3 = Nres * coefPoblacionN3;
